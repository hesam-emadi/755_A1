...
def testing(data_name, X, y, X_test, y_test):
    sss = StratifiedShuffleSplit(n_splits=1, test_size=0.2, random_state=42)

    for train_index, test_index in sss.split(X, y):
        X_train, a = X.loc[[i for i in train_index if i != 0]], X.loc[test_index]
        y_train, a = y[[i for i in train_index if i != 0]], y[test_index]
    ...
    scaler = preprocessing.StandardScaler().fit(X_train)
    X_train, a = scaler.transform(X_train), scaler.transform(X_test)
    ...
    scoring = {'accuracy': 'accuracy', 'r2': 'r2'}
    for model_type in models:
        grid_search = graph_grid_search(model_type['title'], [model_type['x_param'], 'Score'], model_type['model'],
                                        model_type['params'],
                                        scoring, X_train, y_train, model_type['x_param'])
        classifier = grid_search.best_estimator_
        ...
        print('-' * 100)
        print('{} Testing accuracy: {:.2f}%.'.format(model_type['title'], 100 * accuracy_score(y_test, predictions)))
